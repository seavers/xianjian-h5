...
import { loadRgm, loadMgo, loadGop, loadBall, loadMgoCount, loadMsg, loadFbp, loadWord } from '../resources/pal.js';
import { state } from '../engine/state.js';
import { scriptCodes } from '../engine/command.js';

// ==================== 🎒 物品资料模块高级解码辅助函数 ====================
let selectedItemId = 99; // 默认选中 “桂花酒”

// 根据 wordId 从 jianti.fon 点阵字库渲染像素名字 Canvas
function getWordImg(wordId, color = 0xFFFFFF) {
...
// 获取物品点阵汉字名称 HTML（采用 dataurl 优雅内嵌预览，完美防乱码）
...
// 核心物品 20 项高级解构映射表（移植自 index.html 底层）
...
function renderItemTab(container) {
  ...
  items.forEach(item => {
    ...
    leftHtml += `
      <div data-item-item="${item.id}" onclick="onGameDataItemSelect(${item.id})" style="padding: 8px 12px; background: ${isSelected ? 'rgba(255, 215, 0, 0.08)' : 'rgba(255,255,255,0.015)'}; border: 1px solid ${isSelected ? 'var(--glow-yellow)' : 'rgba(255,255,255,0.03)'}; border-radius: 2px; cursor: pointer; display: flex; align-items: center; justify-content: space-between; transition: all 0.15s;">
        <span style="font-size: 9px; font-weight: bold; color: ${isSelected ? 'var(--glow-yellow)' : '#fff'}; display: flex; align-items: center; gap: 4px;">
          ${nameHtml}
        </span>
  ...
  // 右侧物品高级解构详情
  const item = items[selectedItemId] || items[99];
  const rightHtml = `
...
function updateItemSelection() {
  ...
  // 设置新选中样式
  const activeEl = container.querySelector(`[data-item-item="${selectedItemId}"]`);
  if (activeEl) {
    activeEl.style.background = 'rgba(255, 215, 0, 0.08)';
    activeEl.style.borderColor = 'var(--glow-yellow)';
    const span = activeEl.querySelector('span');
...
function drawItemBall(itemId) {
  try {
    ...
    console.error('绘制物品小图标 Ball 失败:', e);
  }
}
...
function buildItemRightHtml(item) {
  ...
  const usescrHtml = item.useScr > 0
    ? `<span onclick="jumpToGameDataScript(${item.useScr})" style="color: var(--glow-yellow); text-decoration: underline; cursor: pointer; font-weight: bold;">Script #${item.useScr} ➔ 穿梭反解</span>`
    : '<span style="color: rgba(255,255,255,0.25);">无 (0)</span>';

  const equscrHtml = item.equScr > 0
    ? `<span onclick="jumpToGameDataScript(${item.equScr})" style="color: var(--glow-yellow); text-decoration: underline; cursor: pointer; font-weight: bold;">Script #${item.equScr} ➔ 穿梭反解</span>`
    : '<span style="color: rgba(255,255,255,0.25);">无 (0)</span>';

  const dropscrHtml = item.dropScr > 0
    ? `<span onclick="jumpToGameDataScript(${item.dropScr})" style="color: var(--glow-yellow); text-decoration: underline; cursor: pointer; font-weight: bold;">Script #${item.dropScr} ➔ 穿梭反解</span>`
    : '<span style="color: rgba(255,255,255,0.25);">无 (0)</span>';
  ...
  return `
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,215,0,0.15); padding-bottom: 8px; margin-bottom: 12px;">
      <div style="display: flex; align-items: center; gap: 8px;">
        <h2 style="margin: 0; font-size: 12px; color: var(--glow-yellow); font-weight: bold; display: flex; align-items: center; gap: 6px;">
          ${nameHtml}
        </h2>
    ...
    <div style="flex: 1; display: flex; gap: 15px; overflow: hidden;">
      ...
      <div style="width: 180px; display: flex; flex-direction: column; gap: 10px; align-items: center; background: rgba(0,0,0,0.4); padding: 12px; border: 1px solid rgba(255,255,255,0.02); border-radius: 3px;">
        ...
        <div style="font-size: 7.5px; color: rgba(255,255,255,0.35); text-align: left; line-height: 1.4; width: 100%;">
          数据偏移: ${info.offset}<br>
          物品 Flags: ${info.flags}<br>
          是否消耗: ${info.consumable}<br>
          是否丢弃: ${info.throwable || '是'}<br>
          是否可售: ${info.sellable || '是'}
        </div>
      </div>
      ...
      <div style="flex: 1; display: flex; flex-direction: column; gap: 12px; overflow-y: auto; padding-right: 4px;">
        <div>
          <div style="font-size: 8.5px; color: rgba(255,255,255,0.4); font-weight: bold; margin-bottom: 6px; display: flex; align-items: center; gap: 4px;">
            <span style="width: 3px; height: 3px; background: var(--glow-yellow); border-radius: 50%;"></span> 物品基础属性
          </div>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px;">
        ...
        <div>
          <div style="font-size: 8.5px; color: rgba(255,255,255,0.4); font-weight: bold; margin-bottom: 6px; display: flex; align-items: center; gap: 4px;">
            <span style="width: 3px; height: 3px; background: var(--glow-yellow); border-radius: 50%;"></span> 装备增益参数
          </div>
